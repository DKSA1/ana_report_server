...
async def handle(group, task):
    ...
    #         and_(
    #             ebay_custom_report_task.c.task_id == hy_task.task_data['task_id'],
    #             ebay_custom_report_task.c.user_id == hy_task.task_data['user_id']
    #         )
    #     ).values({
    #         # ebay_custom_report_task.c.status: 0,
    #         ebay_custom_report_task.c.update_time: (datetime.now()).strftime('%Y-%m-%d %H:%M:%S')
    #     })
